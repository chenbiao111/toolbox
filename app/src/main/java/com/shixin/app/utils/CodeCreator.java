...
public class CodeCreator {
    ...
    /*生成二维码*/
    public static Bitmap createQRCode(String content, int w, int h, String qjcolor, String bjcolor, Bitmap logo) {
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    if (x >= offsetX && x < offsetX + logoW && y >= offsetY && y < offsetY + logoH) {
                        ...
                        if (pixel == 0) {
                            if (matrix.get(x, y)) {
                                pixel = Color.parseColor(qjcolor);
                            } else {
                                pixel = Color.parseColor(bjcolor);
                            }
                        }
                        pixels[y * w + x] = pixel;
                    } else {
                        if (matrix.get(x, y)) {
                            pixels[y * w + x] = Color.parseColor(qjcolor);
                        } else {
                            pixels[y * w + x] = Color.parseColor(bjcolor);
                        }
                    }
                }
            }
